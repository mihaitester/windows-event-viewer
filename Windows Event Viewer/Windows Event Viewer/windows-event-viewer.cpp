...
{
    wprintf(L"\nPrintHelp:\n");
    wprintf(L"\"%s\" [PathToEventFile] [XPathFormattedQuery] [PathToDumpFile] [SuffixToAppendToFilename]\n", argv[0]);
    wprintf(L"EXAMPLE: \"%s\" c:\\Windows\\System32\\Winevt\\Logs\\Security.evtx Event/System[EventID=4624] c:\\Users\\%%username%%\\Downloads -An-account-was-successfully-logged-on\n", argv[0]); // c:\Windows\System32\Winevt\Logs\Security.evtx Event/System[EventID=4624] c:\Users\%username%\Downloads
    wprintf(L"EXAMPLE: \"%s\" %%SystemRoot%%\\System32\\Winevt\\Logs\\Security.evtx Event/System[EventID=4624] c:\\Users\\%%username%%\\Downloads -An-account-was-successfully-logged-on\n", argv[0]); // %SystemRoot%\System32\Winevt\Logs\Security.evtx Event/System[EventID=4624] c:\Users\%username%\Downloads
}
...
int wmain(int argc, wchar_t* argv[], wchar_t* envp[])
{
    ...
    swprintf_s(buffer, MAX_PATH_LONG, L"QueryEvents(%s,%s,%s)", pwsLogFile, argv[2], pwsDumpFile);
    TIMEIT_PRETTY(buffer, status = QueryEvents(pwsLogFile, argv[2], pwsDumpFile));
